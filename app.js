...
async function syncIntervals(force=false){
  ...
  try{
    ...
    const autoCount = saveIntervalsData(data.snapshot?data:{snapshot:data});
    toast((force?'Intervals: zsynchronizowano live ✅':'Intervals: dane odświeżone ✅') + (autoCount ? ` • uzupełniono ${autoCount} treningów` : ''));
    renderAll();
...
function extractIntervalsActivities(data){
  const snap = data?.snapshot || data || intervalsSnapshot() || {};
...
function openWorkout(id){const it=items.find(x=>x.id===id); if(!it)return; $('modalTitle').textContent=it.discipline; $('modalSub').textContent=`${fmtDate(it.date)} • ${it['Dzień']||''} • ${it.week}`; const l=getLog(id); $('modalBody').innerHTML=`${workoutFull(it)}<div class="section-title"><div><h3>Wpis po treningu</h3></div></div>${l.updated?`<div class="detail-grid"><div class="detail-item"><b>RPE / ból / sen</b><p>RPE ${esc(l.rpe||'—')} • ból ${esc(l.pain||'—')} • sen ${esc(l.sleep||'—')} h</p></div><div class="detail-item"><b>Dane</b><p>${esc(l.time||'—')} min • ${esc(l.km||'—')} km • ${esc(l.shoes||'—')}</p></div><div class="detail-item"><b>Paliwo</b><p>${esc(l.fuel||'—')}</p></div><div class="detail-item"><b>Uwagi</b><p>${esc(l.notes||'—')}</p></div></div>`:'<div class="empty">Ten trening nie ma jeszcze wpisu.</div>'}<div style="margin-top:16px"><button class="btn primary" onclick="closeModal();prefillLog('${id}')">Uzupełnij / edytuj</button></div>`; $('detailModal').classList.add('active')}
